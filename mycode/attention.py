...
class _BaseMultiHeadAttention(Layer):
    ...
    def mask_attention_if_needed(self, dot_product):
        ...
        result =  K.constant(low_triangle_ones, dtype=K.floatx()) * dot_product \
...
class MultiHeadSelfAttention(_BaseMultiHeadAttention):
    ...
    def call(self, inputs, **kwargs):
        ...
        #     raise ValueError(
        #         'The layer can be called only with one tensor as an argument')# 这里好像是keras版本问题，没有is_tensor()
...
# 检索全局定制类，推荐利用custom_object_scope更新和清理定制对象，
# 但get_custom_objects可被直接用于访问_GLOBAL_CUSTOM_OBJECTS。
# 本函数返回从名称到类别映射的全局字典。